...
const MarketInsights = () => {
    ...
    const keyStats = [
        {
            ...
            value: "$1.24M",
            icon: (
                <svg className="w-5 h-5 md:w-6 md:h-6 text-white/40" viewBox="0 0 24 24" fill="none" stroke="currentColor">
    ...
    return (
        <section className="bg-black text-white py-12 sm:py-16 md:py-24 relative overflow-hidden">
            {/* Background Elements */}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
                ...
                <div className="mb-8 md:mb-16">
                    ...
                    <h2 className="text-4xl sm:text-5xl md:text-6xl font-extralight tracking-wider mb-4 md:mb-6">
                        Market Insights
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 mb-8 md:mb-16">
                    {marketMetrics.map((metric, index) => (
                        <motion.div
                            ...
                            className="bg-neutral-900/50 backdrop-blur-sm rounded-xl md:rounded-2xl p-4 sm:p-6 md:p-8 border border-white/5"
                        >
                            <div className="flex justify-between items-start mb-6 md:mb-8">
                                <div>
                                    <h3 className="text-xl md:text-2xl font-light mb-2">{metric.title}</h3>
                                    <span className="text-3xl sm:text-4xl md:text-5xl font-light">{metric.value}</span>
                                </div>
                                <span className="text-green-400 text-base md:text-lg">{metric.trend}</span>
                            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                    {keyStats.map((stat, index) => (
                        <motion.div
                            ...
                            className="group relative bg-neutral-900/50 backdrop-blur-sm rounded-lg md:rounded-xl p-4 md:p-6 border border-white/5 hover:border-white/20 transition-all duration-300"
                        >
                            <div className="flex items-start space-x-3 md:space-x-4">
                                <div className="p-1.5 md:p-2 rounded-lg bg-white/5 group-hover:bg-white/10 transition-colors duration-300">
                                    {stat.icon}
                                </div>
                                <div>
